...
function getStatus_(p) {
  ...
  /* マイページを開かれたときにも、期限切れを片づける（v11.3）。
     これまでは受付フォームを開いた人がいるときだけだった。
     15人で埋まったあとは受付フォームを開く人が減り、期限切れの処理が
     毎時の自動処理まで最大1時間遅れる。そのぶん繰上げも遅れてしまう。
     30秒に1回までなので、8秒ごとの自動確認から呼ばれても重くならない。 */
  sweepIfNeeded_(found.seminarKey);
