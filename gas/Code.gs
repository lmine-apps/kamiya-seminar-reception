/*** 神谷梓さん 受付管理システム GAS v11.10 **************************
 * 【v11.10 2026-09-13】セルフの繰上げ猶予を 3日 → 48時間
 *   9/18開催まで日がなく、猶予3日のままだと、いま繰り上がった方の期限
 *   （9/16夕方）が切れたあと、次の方の期限が 9/19＝開催後になってしまう。
 *   48時間なら 2人目は9/18（開催当日）。1人目が早くお支払いになれば前倒しになる。
 *   ※すでに繰り上がっている方には影響しない（期限はF列に書き込み済みで、
 *     この設定は繰り上げた瞬間に1回だけ使われるため）。
 *
 ...
 *   繰り上がった方（申込日時が何日も前）は、お選びいただいた瞬間に
 *   期限切れの時刻が入ってしまっていた。
 *   計算した期限がすでに過ぎている場合は、いまから数え直すようにした。
 *
 ...
 *   返事に乗らず、アプリには「決済案内中」のままに見えていた。
 *   （本番のテストで、20分過ぎてもスプレッドシートが変わらないことを確認）
 *
/*** 神谷梓さん 受付管理システム GAS v11.3 ***************************
 * 【v11.3 2026-09-02】マイページを開いたときにも期限切れを片づける
 *   これまでは受付フォーム（get_capacity）だけで掃除していたため、
 *   15人で埋まったあとは処理が毎時の自動処理まで最大1時間遅れ、
 *   キャンセル待ちの繰上げも同じだけ遅れていた。
 *   get_status からも sweepIfNeeded_ を呼ぶ（30秒に1回まで）。
 *
...
var SEMINARS = {
  ...
  'self_priority':   { sheet: '📋 セルフ先行',  capacity: 19, payment: 'cash',    open_at: '2026-08-20 00:00',  // 先行は8/20から受付中
                       ...
                       /* 繰上げの猶予は48時間（v11.10 2026-09-13）。
                          9/18開催まで日がなく、3日のままだと次の方の期限が
                          開催後になってしまうため短くした。 */
                       promote_deadline_min: 2880,
  ...
  'self_general':    { sheet: '📋 セルフ一般',  capacity: 30, payment: 'prepaid', open_at: '2026-08-24 21:00',
                       ...
                       /* 繰上げの猶予は48時間（v11.10 2026-09-13）。
                          9/18開催まで日がなく、3日のままだと次の方の期限が
                          開催後（9/19）になってしまうため短くした。
                          すでに繰り上がっている方の期限はF列に書き込み済みなので、
                          この変更の影響を受けるのは、これから繰り上がる方だけ。 */
                       promote_deadline_min: 2880 },
...
function getStatus_(p) {
  ...
  /* 行を読む前に、期限切れを片づけておく（v11.4）。
     読んだあとに掃除すると、掃除の結果が返らず、アプリには
     「決済案内中」のままに見えてしまう（本番のテストで実際に発生）。
     30秒に1回までなので、8秒ごとの自動確認から呼ばれても重くならない。 */
  if (p.seminar && SEMINARS[p.seminar]) sweepIfNeeded_(p.seminar);
...
function choosePayment_(p) {
  ...
  if (method === 'bank') {
    ...
    /* 計算した期限がすでに過ぎていたら、いまから数え直す（v11.6）。
       キャンセル待ちから繰り上がった方は、申込日時が何日も前になる。
       そのまま「申込日時＋24時間」で入れてしまうと、お選びいただいた
       瞬間に期限切れの時刻が入り、カードへ戻すこともできなくなる。
       ※お支払い方法の変更は1回までなので、これで期限を延ばし続けることはできない。 */
    if (newDate.getTime() <= Date.now()) {
      newDate = deadlineFrom_(config, new Date(), 'bank');
    }
  } else {
